...
import { safeProjectHrefs } from "@/lib/safe-href"
        ...
        {featuredProjects.length > 0 && (
          <div className="grid lg:grid-cols-2 gap-8 mb-16">
            {featuredProjects.map((project, index) => {
              const links = safeProjectHrefs(project)
              const hasActions = Boolean(links.liveUrl || links.githubUrl)

              return (
                <motion.div key={project.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.2, duration: 0.8 }}>
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
                          {links.liveUrl && (
                            <Button size="sm" variant="secondary" asChild>
                              <a href={links.liveUrl} target="_blank" rel="noopener noreferrer">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {otherProjects.map((project, index) => {
                ...
                return (
                  <motion.div key={project.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1, duration: 0.6 }}>
